...
type Rack struct {
	...
	// MaxUnavailable opts this cluster in to one PodDisruptionBudget PER RACK,
	// and sets the maximum number of pods in THIS rack that may be voluntarily
	// disrupted at once. Setting it on any rack switches the whole cluster to the
	// per-rack shape; racks that leave it unset then get replication-factor - 1,
	// measured against their own effective aerospikeConfig and floored at 1.
	//
	// Leave it unset — the default — to get a single cluster-wide
	// PodDisruptionBudget. Kubernetes evaluates per-rack budgets independently,
	// so with them the cluster's concurrent-eviction bound is the SUM of the
	// racks' budgets, not any single number. A rack is a scheduling topology on
	// Community Edition, not a data-placement unit (namespace rack-id is
	// Enterprise-only), so a partition's copies can sit on any two nodes
	// regardless of rack and the cluster-wide budget is the meaningful one.
	// +optional
