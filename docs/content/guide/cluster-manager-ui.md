...
- **Monitoring** — Prometheus exporter, ServiceMonitor, PrometheusRule, exporter 환경 변수 설정
...
- **Reset Button** — 서킷 브레이커 수동 리셋 (no-op 패치로 재시도 트리거). 실패 원인을 해결한 후 이 버튼을 클릭하면 `failedReconcileCount`가 0으로 초기화되고 즉시 재조정이 시작됩니다.
