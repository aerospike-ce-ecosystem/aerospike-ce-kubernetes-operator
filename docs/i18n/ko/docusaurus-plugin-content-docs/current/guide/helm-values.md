...
| `image.repository` | string | `ghcr.io/kimsoungryoul/aerospike-ce-kubernetes-operator` | 오퍼레이터 컨테이너 이미지 리포지토리. |
...
| `ui.image.repository` | string | `ghcr.io/kimsoungryoul/aerospike-cluster-manager` | UI 컨테이너 이미지 리포지토리. |
